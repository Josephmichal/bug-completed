...
-->ini ivan sql i nokunu. ie, username=a'%password=a' or a'-- ,  a' OR 1=1-- (url encode)angane nokunu apol 200 ok enn thane enan kanikunath.chelapo blind sql i undakam
...
-->oru image right click chyth inspect element adikuka enit athile code il change varuthuka ie, <mg src= x onerror=alert(1) style='width:300px' > (img enan kodukendath angane kodukumbol obsedian pani tharunu) 
...
-->result il 302 verunundengil ath working aayi ennayirikum so sredhikuka.ivide ee 
...
-->nalla csrf aanengil namal email change chyuna request send chythal kitiya response il puthiya csrf aayirikum undavuk.athupole nalla csrf aanengil ath session cookie yumayi mingle chythitundavum ennan ivan parayunath
...
-->enit mukalillula target: enna sthalath thane add kodukuka enit payload il nerthethe subdomains kodukuka enit resourse pool il poi maximum concurrent : 500 kodukuka(karanam 500 subomains aan ullath athukond.500 req ottadik poyal ellam block chyan onum patila athukond)
...
-->ipol namal subdomains intruder il payload aayi itt chythu ithupole oro subdomains inum ip address undakum so oru ip eduth ipinfo.io il poi iptange nokuka so ini nerthepole target:https://69.145.183.$2$ koduth payload type numbers kodukuka enit 1:255:1 enna format kodukuka enit start attack adich nokam.angane oro ip ilum entha nadakunath enn nokam
-->ee kitiya subdomains orornum check chyumbol google/duckduckgo il poi site:subdomain "token"        angane oro keywords use chyth search chyuka.
...
-->subdomains enumerate chythit ella subdomain um copy chyth intruderil idanam.aadyam target: $https://twitte.com$ idanam enit GET /$important-files$ idanam.
